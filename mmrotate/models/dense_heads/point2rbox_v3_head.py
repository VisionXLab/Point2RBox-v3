...
class Point2RBoxV3Head(AnchorFreeHead):
    ...
    def __init__(self,
                 num_classes: int,
                 in_channels: int,
                 strides: list = [8, 16, 32, 64, 128],
                 regress_ranges: list = [(-1, 64), (64, 128), (128, 256), (256, 512), (512, INF)],
        ...
        self.use_adaptive_scale = kwargs["use_adaptive_scale"]
        del kwargs["use_adaptive_scale"]
        super().__init__(
            num_classes,
            in_channels,
            strides=strides,
    ...
    def forward(
        ...
        Args:
            feats (tuple[Tensor]): Features from the upstream network, each is
                a 4D-tensor, particularly, in point2rboxv2 fpn version, len(x) == 5.
    ...
    def get_targets(
        ...
        num_points = [center.size(0) for center in points]  # [16384, 4096, 1024, 256, 64]
    ...
    def _get_targets_single(
        ...
        """Compute regression and classification targets for a single image all level fpn."""
        ...
        gt_bboxes = gt_bboxes[None].expand(num_points, num_gts, 5)  # (num_gts, 5) ---> (num_points, num_gts, 5)
        
        gt_ctr, gt_wh, gt_angle = torch.split(gt_bboxes, [2, 2, 1], dim=2)  # (num_points, num_gts, 2), (num_points, num_gts, 2), (num_points, num_gts, 1)
        
        cos_angle, sin_angle = torch.cos(gt_angle), torch.sin(gt_angle)
        rot_matrix = torch.cat([cos_angle, sin_angle, -sin_angle, cos_angle],
                               dim=-1).reshape(num_points, num_gts, 2, 2)
        
        offset = points - gt_ctr  # (num_points, num_gts, 2)
        offset = torch.matmul(rot_matrix, offset[..., None])
        offset = offset.squeeze(-1)  # (num_points, num_gts, 2)
        
        gt_w, gt_h = gt_wh[..., 0].clone(), gt_wh[..., 1].clone()
        
        ## center_r = torch.clamp((w * h).sqrt() / 64, 1, 5)[..., None]
        offset_x, offset_y = offset[..., 0], offset[..., 1]
        
        left = gt_w / 2 + offset_x  # based point coordication
        ...
        if self.center_sampling:
            ...
            stride = offset.new_zeros(offset.shape)  # (num_points, num_gts, 2)

            # project the points on current lvl back to the `original` sizes
            lvl_begin = 0
            for lvl_idx, num_points_lvl in enumerate(num_points_per_lvl):
                lvl_end = lvl_begin + num_points_lvl
                stride[lvl_begin:lvl_end] = self.strides[lvl_idx] * radius  # 8*0.75, 16*0.75, 32*0.75, 64*0.75, 128*0.75
                lvl_begin = lvl_end
        ...
        inside_regress_range = (
            ...
            & (max_regress_distance <= regress_ranges[..., 1]))  # (-1, 64), (64, 128), (128, 256), (256, 512), (512, INF)
        ...
        labels = gt_labels[min_area_inds]  # (num_points, num_class) 
    ...
    def predict_by_feat(
        ...
        # particually, [(128, 128), (64, 64), (32, 32), (16, 16), (8, 8)]
    ...
    def selest_pseudo(self, results_list: List[InstanceData]) -> InstanceData:
        ...
        scores_all = torch.cat(scores_list, dim=1).sigmoid()  # (all_num_points, num_clsss)
        max_values, max_ids = torch.max(scores_all, dim=1)  # select best among fpn
        
        bboxes_stacked = torch.stack(bboxes_list, dim=0)  # (fpn_level_num, obj_num, 5)
        labels_stacked = torch.stack(labels_list, dim=0)  # (fpn_level_num, obj_num, 1)
        
        row_indices = torch.arange(bboxes_stacked.size(1))  # (obj_num,)
    
        pseudo_bboxes_selected = bboxes_stacked[max_ids, row_indices, :]
    ...
    def _predict_by_feat_single_pseudo(self,
        ...
        if self.training:
            mask = gt_instances.bids[:, 1] == 0
            gt_bboxes = gt_instances.bboxes.tensor[mask]
            gt_labels = gt_instances.labels[mask]
        else:
